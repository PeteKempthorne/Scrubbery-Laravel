...
<br />
    <div class="row no-pad row-eq-height"> 
        <div class="col-md-9">
            ...
            $totalArticles = DB::table('articles')->count(); //number of articles in db
            ...
            $pagSQL = DB::table('articles')->orderBy('ArticleID', 'desc')->offset($offset)->limit($articlesPerPage)->get();
           
            foreach ($pagSQL as $row)
            {                
                $title = $row->Title;
                $date = $row->Date;
                $image = $row->Image;
